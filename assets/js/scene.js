...
function SakuraRain(scene) {
    ...
    for(let i = 0; i < 50; i++) {
      Konva.Image.fromURL(sakuraImgData, function (darthNode) {
        ...
        darthNode.setAttrs({
          ...
          scaleX: x,
          scaleY: x,
          offsetX: random(0, 200),
          offsetY: random(0, 200),
          opacity: 0,
  ...
  function animation() {
    for(let i = 0; i < sakuraList.length; i++) {
      ...
      nodeData.node.position(p)
      nodeData.node.rotation((nodeData.node.rotation() % 360) + nodeData.rotateSpeed)
    }
